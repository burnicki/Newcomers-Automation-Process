        ...
        try:
            ...
            if excel_employee_name == process_string(sulu_employee_name):
                ...
                for sharepoint in sharepoint_data:
                    if sharepoint[0] == excel_employee_id:
...
def filter_employee_data(employee_data, shippment_data):
    employee_df = pd.DataFrame(employee_data, columns=['entra_id', 'employee_name', 'employee_id', 'employee_start_date', 'employee_personal_mail', 'employee_one_password'])
    shippment_df = pd.DataFrame(shippment_data)
    
    # Filtracja employee_data na podstawie obecności employee_id w shippment_data
    filtered_employee_df = employee_df[employee_df['employee_id'].isin(shippment_df['employeeID'])]
    filtered_employee_data = filtered_employee_df.values.tolist()
    
    # Filtracja shippment_data na podstawie obecności employee_id w employee_data
    filtered_shippment_df = shippment_df[shippment_df['employeeID'].isin(filtered_employee_df['employee_id'])]
    filtered_shippment_df = filtered_shippment_df[['name', 'address', 'phone']]
    
    return filtered_employee_data, filtered_shippment_df
...
def check_email_tracker_list(employee_data, site_id, email_tracking_list_id, headers, mail_sender, shippment_data, office_pick_up, equipment_data):
    ...
    # Filtruj employee_data i shippment_data
    employee_data, shippment_data = filter_employee_data(employee_data, shippment_data)
        ...
        if start_date - timedelta(days=4) <= current_date <= start_date:
            for data in result["value"]:
                ...
                if employee_id == list_employee_id:
                    ...
                    filtered_employee_ids.append(employee_id)
                    break
            if not already_on_list:
                employee_details_to_add.append(employee)
    
    if employee_details_to_add:
        # Usuń pracowników, którzy są już na liście, z shippment_data
        shippment_data = shippment_data[~shippment_data['employeeID'].isin(filtered_employee_ids)]
